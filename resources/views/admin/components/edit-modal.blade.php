<div class="modal fade" id="{{ $table . 'EditModal' }}" tabindex="-1" aria-labelledby="{{ $table . 'EditModalLabel' }}"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="{{ $table . 'EditModalLabel' }}">Edit {{ $table }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="{{ $table . 'EditForm' }}" method="POST" action="{{ '/' . $table . '/update/:id' }}">
                @csrf
                @method('PUT')
