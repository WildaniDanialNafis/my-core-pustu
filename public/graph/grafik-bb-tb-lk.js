let dataGrafikBbTbLk = null;

function getCsrfToken() {
    return document.querySelector('meta[name="csrf-token"]')?.content || '';
}

async function loadGrafikBbTbLk() {
    const csrfToken = getCsrfToken();

    try {
        const response = await fetch('/grafik/data-grafik-bb-tb-lk/data', {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) throw new Error('Gagal fetch ke /ajax/data-grafik-bb-tb-lk');

        const data = await response.json();
        dataGrafikBbTbLk = data;

        await renderGrafikBbTbLk(csrfToken);
    } catch (error) {
        console.error('Error:', error);
    }
}

// async function renderGrafikBbTbLk(csrfToken) {
...
//     try {
//         const response = await fetch('/ajax/grafik-bb-tb-lk', {
//             method: 'GET',
//             headers: {
//                 'X-Requested-With': 'XMLHttpRequest',
//                 'X-CSRF-TOKEN': csrfToken
...
//         initializeChartsBbTbLk(dataGrafikBbTbLk);
...
// function initializeChartsBbTbLk(data) {
