let dataGrafikImtPr = null;

function getCsrfToken() {
    return document.querySelector('meta[name="csrf-token"]')?.content || '';
}

async function loadGrafikImtPr() {
    const csrfToken = getCsrfToken();

    try {
        const response = await fetch('/ajax/data-grafik-imt-pr', {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) throw new Error('Gagal fetch ke /ajax/data-grafik-imt-pr');

        const data = await response.json();
        dataGrafikImtPr = data;

        await renderGrafikImtPr(csrfToken);
    } catch (error) {
        console.error('Error:', error);
    }
}

async function renderGrafikImtPr(csrfToken) {
    ...
    try {
        const response = await fetch('/ajax/grafik-imt-pr', {
            method: 'GET',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'X-CSRF-TOKEN': csrfToken
        ...
        initializeChartsImtPr(dataGrafikImtPr);
...
function initializeChartsImtPr(data) {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    intersect: false,
                    callbacks: {
                        label: context => `IMT: ${context.parsed.y} kg/m²`
                    }
                }
            },
            scales: {
    ...
    const trafficChart = new Chart(trafficCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: context => `${context.label}: ${context.parsed} kg/m²`
                    }
                }
            }
        }
    ...
    revenueCanvas.addEventListener('mousemove', evt => {
        ...
        if (points.length && chartHoverInfo) {
            ...
            chartHoverInfo.textContent = `${label}: ${value.toLocaleString()} kg/m²`;
